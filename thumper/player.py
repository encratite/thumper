...
class ThumperPlayer:
	def __init__(self):
		self.spice = 0
		self.solari = 0
		self.troops_garrison = Constant.INITIAL_TROOPS
		self.troops_deployed = 0
		self.influence = 0
		self.swordmaster = False
		self.palace = False
		self.holtzman_shield = False
		self.agents_left = Constant.INITIAL_AGENTS
	...
	def reset(self):
		...
		self.troops_deployed = 0
		action_type_count = Constant.ACTION_TYPES
		if self.swordmaster:
			action_type_count += 1
	...
	def get_reward(self):
		reward = self.victory_points - self.previous_victory_points
		if reward > 0:
